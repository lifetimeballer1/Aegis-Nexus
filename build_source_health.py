...
def main():
 ...
 for name,r in results.items():
  ...
  sources.append({'name':name,'url':source_url,'type':r.get('type','social'),'category':r.get('category','osint'),'coverage':list(r.get('coverage') or []),'status':'online' if ok else 'failed','contentStatus':'data_available' if rows else ('online_empty' if ok else 'unavailable'),'quarantined':name in QUARANTINE,'quarantineNote':QUARANTINE.get(name,{}).get('note','') if name in QUARANTINE else '','lastChecked':current.get('updatedAt') or now.isoformat(),'lastSuccess':current.get('updatedAt') if ok else None,'freshnessMinutes':age_minutes(current.get('updatedAt'),now) if ok else None,'rowsFetched':rows,'newArticles':int(r.get('newArticles',0) or 0),'mode':r.get('mode'),'consecutiveFailures':0 if ok else 1,'error':r.get('error',''),'fallbackAvailable':False,'dataValue':'posts' if rows else 'none'})
