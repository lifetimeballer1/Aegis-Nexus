...
## Intelligence Web Concept Track W1–W40 (2026-09-10, branch `intelweb-concept-track`) — DONE
- Concept tokens/chrome inside the iframe, fluid frame height; honest legend and all canonical node kinds colored; co-mention vs action edge encoding; camera-preserving search with relationship/evidence filters and date scrubber; dialog semantics + focus trap; relation direction/confidence/event IDs; BFS path tracing, isolate, device-local pins; keyboard traversal, reduced motion, edge tooltips, focus camera; lazy iframe restored (runtime eager override removed), runtime budget, hidden-tab pause, fetch cache, WebGL context-loss handling, action-overlay fix + toggle; postMessage selection bridge and Map/Brain cross-links; explorer sort/paging; doc drift corrected.
- Verified: pytest 116, node filter tests 7, repository/security/performance validators, `intelligence_web_smoke` RENDER/CONTROLS/FAILURE PASS, `dashboard_smoke` desktop+mobile, custom inspect + bridge checks, zero page errors.

## Global Map Concept Track M5–M44 (2026-09-10, branch `map-concept-track`) — DONE
- Concept chrome; detail drawer with real fields + linked-event candidates + Brain caveat; selected-marker highlight; confidence-derived severity; ops sorting/severity filters/debounce/selection sync; OSINT chip; layer-panel consistency and dismissal; regional/strategic/cartel reference panels (no invented coordinates); `loadMapData` hydrates from core state (double-fetch removed); memoized collect; zoom-aware mini-map sampling + honest wording; stale-feed badge; a11y roles/live regions/copy coords/touch targets/reduced motion.
- Verified: pytest 116 (M1–M4 contracts intact), node filter tests 7, validators, `dashboard_smoke` desktop+mobile, all Web smokes, zero page errors.
- Remaining: region polygons and enforcer pins impossible from available data; `validate_operational_health` blocked only by pre-existing snapshot staleness.

## Brain Track B1–B40 (2026-09-10, branch `brain-track`) — DONE
- Made the Intelligence Brain the site-wide fusion layer: story graph from canonical event clusters (`brain_stories.json`) with eight evidence-backed gap detectors and a refresh-driven gap lifecycle (`brain_gap_history.json`, open → narrowing → closed with closing evidence). Rewired the dormant live-events/claims/source-evidence/event-intelligence/assessment/trends layers into the canonical refresh with fresh gates and manifest coverage.
